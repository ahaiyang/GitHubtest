...
--向上移动的提示文字
function MUtils.showFlyWord(strText, color, nfontSize, imgBg )
    local runningScene = cc.Director:getInstance():getRunningScene()
    if runningScene then
        local path = imgBg or "common/img_pop_fly_word_bg.png"
        ...
        local label = cc.Label:createWithSystemFont(strText or "温馨提示", "", nfontSize or 40)
        label:setAnchorPoint(cc.p(0.5, 0.5))
        label:setPosition(cc.p(size.width/2, size.height/2))
        label:setTextColor(color or cc.c3b(236,236,23))
        bgSprite:addChild(label)
        bgSprite:setLocalZOrder(10000)
        runningScene:addChild(bgSprite)
        local callFunc = cc.CallFunc:create(function(sender)
            bgSprite:removeFromParent()
        end)
        local moveToAction = cc.MoveBy:create(1, cc.p(0, 80));
        bgSprite:runAction(cc.Sequence:create(moveToAction, callFunc) )
    end
end

function MUtils.readfile(path)
...
function MUtils.writefile(path, content, mode)
    mode = mode or "w+b"
